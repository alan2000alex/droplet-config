...
if [[ -n "$DROPLET_IP" ]]; then
  ...
  if ssh -o ConnectTimeout=5 -o BatchMode=yes -o StrictHostKeyChecking=no \
    -i "$SSH_KEY" "$USERNAME@$DROPLET_IP" exit 2>/dev/null; then
    ...
    for dir in .claude .gemini .codex; do
      rsync -az -e "ssh -o StrictHostKeyChecking=no -i $SSH_KEY" \
        "$USERNAME@$DROPLET_IP:~/$dir/" "$LOCAL_CREDS/$dir/" 2>/dev/null || true
    done
    for keyfile in ed25519 ed25519.pub; do
      rsync -az -e "ssh -o StrictHostKeyChecking=no -i $SSH_KEY" \
        "$USERNAME@$DROPLET_IP:~/.ssh/$keyfile" "$LOCAL_CREDS/.ssh/" 2>/dev/null || true
    done
    rsync -az -e "ssh -o StrictHostKeyChecking=no -i $SSH_KEY" \
      "$USERNAME@$DROPLET_IP:~/.claude.json" "$LOCAL_CREDS/" 2>/dev/null || true

    rsync -az -e "ssh -o StrictHostKeyChecking=no -i $SSH_KEY" \
      "$USERNAME@$DROPLET_IP:~/.gitconfig" "$LOCAL_CREDS/" 2>/dev/null || true
